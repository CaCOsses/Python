...
# These two lines remove the Stocks folder and then recreate it in order to remove old stocks. Make sure you have created a Stocks Folder the first time you run this.
#shutil.rmtree("Bayesian_Logistic_Regression/Stocks/Stocks_Sub")
os.mkdir("Bayesian_Logistic_Regression/Stocks/Stocks_Sub")
...
for interval in list_files:
    ...
    data.to_csv("Bayesian_Logistic_Regression/Stocks/Stocks_Sub/"+Stock_Name+".csv")
    print("Data for " + Stock_Name + " has been substantiated with technical features.")

    
Hold_Results = []
list_files2 = (glob.glob("Bayesian_Logistic_Regression/Stocks/Stocks_Sub/*.csv")) # Creates a list of all csv filenames in the stocks folder
for interval2 in list_files2:
    ...
    data = data.drop(['Five_Day_Observation_Outcome', 'Date', 'Open', 'High', 'Low', 'Close'], axis = 1)
    ...
    for dep in dependents:
        ...
        model = LogisticRegression(solver='liblinear', C=20, multi_class='ovr',random_state=0, max_iter=10000)
